...
export const LogoLoop = memo(({
  ...
  const rootClasses = useMemo(() =>
    cx(
      'relative group',
      isVertical ? 'overflow-visible h-full inline-block' : '',
      '[--logoloop-gap:32px]',
  ...
  const renderLogoItem = useCallback((item, key) => {
    if (renderItem) {
      return (
        <li
          className={cx(
            'flex-none overflow-visible text-[length:var(--logoloop-logoHeight)] leading-[1]',
    ...
    const content = isNodeItem ? (
      <span
        className={cx(
          ...
          scaleOnHover &&
            'transition-transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover/item:scale-200'
        )}
    ...
    const inner = item.href ? (
      <a
        className={cx(
          ...
          'hover:opacity-80 hover:text-blue-400 ',
